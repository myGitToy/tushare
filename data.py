...
def update_60min_temp(code_list=''):
    '''
    [更新小时线数据]######
    函数说明 乔晖 2018/4/23
    具体步骤如下：
    1.获取每日开盘的数据，取得当日交易的股票列表
    2.从[60min]的文件夹中读取已有数据
    3.从[get_k_data]中读取新数据
    4.两者合并去重并排序
    5.写入原有文件
    ...
    for code in code_list:
        ...
        if success:
            #读取成功，进行合并操作
            #处理旧数据
            #df_old=df_old[['date','open','close','high','low','volume','code']]
            df_old.set_index(['date'], inplace = True)  
            print('%s旧文本文件中数据量：%s' % (code,df_old.shape[0]))
            #获取新数据
            df_new = ts.get_k_data('%s' % (code), ktype='D')
            #d_new=df_new[['date','open','close','high','low','volume','code']]
            ...
            #保存数据
            df.to_csv('~/environment/TuShare/data/60min/%s.csv' % (code))
            print('%s总数据量：%s' % (code,df.shape[0]))
            ...
            df_current=df_current[['date','open','close','high','low','volume','code']]
            df_current.set_index(['date'], inplace = True)  
            df_current.to_csv('~/environment/TuShare/data/60min/%s.csv' % (code))
            print('%s总数据量失败后处理：%s' % (code,df_current.shape[0]))
            
def update_60min(code_list=''):
    '''
    [更新小时线数据]######
    函数说明 乔晖 2018/4/23
    具体步骤如下：
    ...
    for code in code_list:
        ...
        if success:
            ...
            #print('%s旧文本文件中数据量：%s' % (code,old_count))
            #获取新数据
            print(code)
            df_new = ts.get_k_data('%s' % (code), ktype='60')
            #d_new=df_new[['date','open','close','high','low','volume','code']]
            df_new.set_index(['date'], inplace = True)  
            #新数据存盘
            ...
            print('60分钟线：%s读取完毕，新增数据量：%s条' % (code,all_count-old_count))
            #return all_count-old_count
            #print('60分钟线：%s总数据量：%s' % (code,df.shape[0]))
            
        else:
            #读取失败，说明目录无文件，直接写入
            #获取新数据并保存
            #print("失败后的处理")
            df_current = ts.get_k_data('%s' % (code), ktype='60')
            #df_current=df_current[['date','open','close','high','low','volume','code']]
            df_current.set_index(['date'], inplace = True)  
            df_current.to_csv('~/environment/TuShare/data/60min/%s.csv' % (code))
            print('60分钟线：新增代码%s，数据量：%s条' % (code,df_current.shape[0]))
            #return df_current.shape[0]

def update_5min(code_list=''):
    '''
    [更新5分钟数据]######
    函数说明 乔晖 2018/4/23
    具体步骤如下：
    ...
    for code in code_list:
        try:
            df_old=pd.read_csv('~/environment/TuShare/data/5min/%s.csv' % (code))
            success=True
        except IOError:
            #没有找到文件
            #print('error')
        ...
        if success:
            ...
            old_count=df_old.shape[0]
            #获取新数据
            df_new = ts.get_k_data('%s' % (code), ktype='5')
            df_new.set_index(['date'], inplace = True)  
            #新数据存盘
            df_new.to_csv('~/environment/TuShare/data/temp/%s_temp.csv' % (code))
            #重新读取新数据
            ...
            df=df.sort_index(ascending = True)
            #保存数据
            df.to_csv('~/environment/TuShare/data/5min/%s.csv' % (code))
            #总数据量
            all_count=df.shape[0]
            print('5分钟线：%s读取完毕，新增数据量：%s条' % (code,all_count-old_count))
        else:
            #读取失败，说明目录无文件，直接写入
            #获取新数据并保存
            df_current = ts.get_k_data('%s' % (code), ktype='5')
            df_current.set_index(['date'], inplace = True)  
            df_current.to_csv('~/environment/TuShare/data/5min/%s.csv' % (code))
            print('5分钟线：新增代码%s，数据量：%s条' % (code,df_current.shape[0]))
def update_30min(code_list=''):
    '''
    [更新30分钟数据]######
    函数说明 乔晖 2018/4/23
    具体步骤如下：
    ...
    for code in code_list:
        try:
            df_old=pd.read_csv('~/environment/TuShare/data/30min/%s.csv' % (code))
            success=True
        except IOError:
            #没有找到文件
            #print('error')
